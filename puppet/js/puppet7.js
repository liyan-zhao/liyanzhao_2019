...
angleRS = [], angleRA = [], angleRH = [], 
angleBT = [], angleLL = [], angleRL = [];
var poses = 0;

$('#freezebutton').click(function() {
    var puppetX = parseInt($('#puppet').offset().left)-window.innerWidth*0.50+330;
    ...
    var matrixrightarm = $("#rightarmrot").css('-webkit-transform');
    if(matrixrightarm !== 'none') {
        var values_rightarm = matrixrightarm.split('(')[1].split(')')[0].split(',');
        var a_rightarm = values_rightarm[0];
        var b_rightarm = values_rightarm[1];
        var angle_rightarm = Math.round(Math.atan2(b_rightarm, a_rightarm) * (180/Math.PI));
        angleRA[count] = angle_rightarm;
        angleRA.toString();
        // $("#angle_rightarm").html("Right Arm angle: " + angleRA);
    } else { var angle_rightarm = 0; 
        angleRA[count] = angle_rightarm;
        angleRA.toString();
        // $("#angle_rightarm").html("Right Arm angle: " + angleRA);
    }

    var matrixrighthand = $("#righthandrot").css('-webkit-transform');
    if(matrixrighthand !== 'none') {
        var values_righthand = matrixrighthand.split('(')[1].split(')')[0].split(',');
        var a_righthand = values_righthand[0];
        var b_righthand = values_righthand[1];
        var angle_righthand = Math.round(Math.atan2(b_righthand, a_righthand) * (180/Math.PI));
        angleRH[count] = angle_righthand;
        angleRH.toString();
        // $("#angle_righthand").html("Right Hand angle: " + angleRH);
    } else { var angle_righthand = 0; 
        angleRH[count] = angle_righthand;
        angleRH.toString();
        // $("#angle_righthand").html("Right Hand angle: " + angleRH);
    }
